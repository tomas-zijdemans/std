...
export interface RetryOptions {
  ...
  signal?: AbortSignal;
  /**
   * Callback invoked when a retry is about to happen, immediately before
   * waiting for the backoff delay.
   *
   * Called only when another attempt will follow: never for the terminal
   * failure that becomes {@linkcode RetryError}, never for errors rejected
   * by `isRetriable`, and never when `signal` is already aborted. An abort
   * during the backoff wait can still cancel a retry after this callback
   * has fired.
   *
   * The return value is ignored and not awaited. If the callback throws,
   * {@linkcode retry} rejects with that error and makes no further attempts.
   *
   * @default {undefined}
   *
   ...
   * @param delay The number of milliseconds to wait before the next attempt,
   * after backoff and jitter have been applied.
   */
 ...
 * ```
 *
 * @example Observing retries with `onRetry`
 * ```ts
 * import { retry } from "@std/async/unstable-retry";
 ...
 * assertEquals(logs, [
 *   "Attempt 1 failed, retrying in 1ms",
 *   "Attempt 2 failed, retrying in 2ms",
 * ]);
 * ```
 *
...
export async function retry<T>(
  ...
  while (true) {
    ...
    try {
      ...
      // An aborted signal means no retry follows, so onRetry must not fire.
      signal?.throwIfAborted();
      ...
      onRetry?.(error, attempt + 1, timeout);
